...
- **Notificação nativa** quando um pedido de compra é atribuído ao comprador logado; tocar na notificação abre a tela do pedido no WebView. Sem Firebase: quem busca os avisos é o próprio serviço de GPS (foreground), que já fica vivo em segundo plano.
...
| `GET /api/buyer-push/pending` | Busca os avisos de pedido atribuído (e os marca como entregues) |
...
- **Notificações**: nenhuma credencial extra — não há Firebase/FCM. O app usa a notificação nativa do Android (`POST_NOTIFICATIONS`), alimentada pelo endpoint `GET /api/buyer-push/pending` consultado pelo serviço de GPS.
